...
publishable_key="$(printf '%s\n' "$status_output" | sed -n 's/^PUBLISHABLE_KEY=//p' | tr -d '"')"
legacy_anon_key="$(printf '%s\n' "$status_output" | sed -n 's/^ANON_KEY=//p' | tr -d '"')"
client_key="${publishable_key:-$legacy_anon_key}"

if [[ -z "${api_url}" || -z "${client_key}" ]]; then
  printf 'Could not resolve local Supabase API_URL or client key.\n'
...
VITE_SUPABASE_PUBLISHABLE_KEY=${client_key}
EOF2
